...
#define S_STRLIB_VERSION "0.1.1"
	...
	/*  Format the provided string into a new, allocated memory location
	   ensuring that the resulting memory is large enough for the data to be
	   held NOTE: It is up to the caller to free the resulting memory! */
	...
	/*  Find and return the last index of char `c` in s; if not found, returns
	 * -1 */
	int s_find_reverse(const char* s, const char c);

	/*  Find the last instance of the substring `sub` in s; returns NULL if not
	   found NOTE: This is needed since strrstr (strstr reverse) is not
	   guaranteed to exist */
	char* s_strrstr(const char* s, const char* sub);

	/*  Find and return the index of the substring `sub` in s; if not found,
	 * returns -1 */
	int s_find_str(const char* s, const char* sub);

	/*  Find and return the last index of the substring `sub` in s; return -1 if
	 * not found */
	int s_find_str_reverse(const char* s, const char* sub);

	/*  Find and return the index of any of the characters in s2; retirm -1 of
	 * not found */
	int s_find_any(const char* s, const char* s2);

	/*  Find and return the last index of any of the characters in s2; retirm -1
	 * of not found */
	...
	/*  Append the contents of s2 onto s1; s1 must be dynamically allocated.
	    ...
	    NOTE:   To use s_append you must call like s1 = s_append(s1, s2) or
	   memory will be leaked. NOTE:   To ensure that s1 is updated to point to
	   the new location after memory realloc, use s_append_alt(&s1, s2)*/
	...
	char*
	s_extract_substring_str(const char* s, const char* sub, size_t length);
	...
	/*  Split the provided string by the delimiter `c`; does not keep empty
	 * strings */
	char** s_split_string_c(const char* s, const char c, int* num);

	/*  Split the provided string by the delimiter `sub`; does not keep empty
	 * strings */
	char** s_split_string_str(const char* s, const char* sub, int* num);

	/*  Split the provided string by any of the delimeters in s2; does not keep
	 * any empty strings */
	...
	/*  Remove duplicate spaces in a string while also trimming and
	 * standardizing whitespaces */
	...
	/*  Find the index of the `idx` delimiter; returns -1 if not found
	    NOTE: idx is 1 based (i.e., use 1 for the first instance, 2 for the
	   second) */
